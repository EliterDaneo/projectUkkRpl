...
@section('content')
    <div class="card mt-4 shadow-lg">
        {{-- Card Header: Menggunakan warna primary dan ikon yang jelas --}}
            ...
            <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('a') is-invalid @enderror" name="a" id="category_id"
                            required>
                            {{-- Memberi value="" agar validasi 'required' bekerja jika tidak dipilih --}}
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="c" class="form-control @error('c') is-invalid @enderror"
                            id="name" value="{{ old('c') }}" placeholder="Contoh: Baju Kaos Pria" required>
                        @error('c')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div id="imageHelp" class="form-text">Maksimal 2MB. Format: JPEG, PNG, JPG, GIF, SVG.</div>
                        @error('d')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        {{-- Mengubah min="3" menjadi min="1" untuk harga yang realistis --}}
                        <input type="number" name="e" class="form-control @error('e') is-invalid @enderror"
                            id="price" value="{{ old('e') }}" min="1" placeholder="Contoh: 50000" required>
                        @error('e')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        {{-- Mengubah min="3" menjadi min="0" untuk stok yang realistis --}}
                        <input type="number" name="f" class="form-control @error('f') is-invalid @enderror"
                            id="stock" value="{{ old('f') }}" min="0" placeholder="Contoh: 100" required>
                        @error('f')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="g" class="form-control @error('g') is-invalid @enderror" id="description" rows="4"
                        placeholder="Masukkan deskripsi produk minimal 3 karakter" required>{{ old('g') }}</textarea>
                    @error('g')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                {{-- Tombol Aksi: Senada dengan form Edit dan Hapus --}}
